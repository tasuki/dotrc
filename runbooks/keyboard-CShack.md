...
Czech keyboard as per http://xakru.com/cshack/

Append the following to `/usr/share/X11/xkb/symbols/cz`:

```
xkb_symbols "hack" {
    name[Group1]="Czech (hack)";

    key <TLDE> {[  grave,         asciitilde,  approxeq,             exclamdown            ]};
    key <AE01> {[  1,             exclam,      section,              trademark             ]};
    key <AE02> {[  2,             at,          registered,           copyright             ]};
    key <AE03> {[  3,             numbersign,  Greek_pi,             numerosign            ]};
    key <AE04> {[  4,             dollar,      EuroSign,             cent                  ]};
    key <AE05> {[  5,             percent,     degree,               U2030                 ]};//permille!
    key <AE06> {[  6,             asciicircum, minutes,              radical               ]};
    key <AE07> {[  7,             ampersand,   seconds,              cuberoot              ]};
    key <AE08> {[  8,             asterisk,    infinity,             dagger                ]};
    key <AE09> {[  9,             parenleft,   singlelowquotemark,   U2211                 ]};//summation!
    key <AE10> {[  0,             parenright,  leftsinglequotemark,  rightsinglequotemark  ]};
    ...
    key <AB08> {[  comma,         less,        periodcentered,       lessthanequal         ]};
...
Edit `/usr/share/X11/xkb/rules/evdev.xml` and add the following under xkbConfigRegistry > layoutList > layout (where configItem.name == cz) > variantList

```
<variant>
  <configItem>
	<name>hack</name>
	<description>Czech (hack)</description>
  </configItem>
</variant>
```

Reload `sudo dpkg-reconfigure xkb-data`. Maybe restart the graphical interface.
